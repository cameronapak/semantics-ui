class BcToaster extends HTMLElement {
  connectedCallback() {
    ...
    const initToast = (element) => {
      ...
      const timeoutDuration = duration !== -1 ? duration || (element.dataset.category === 'error' ? 5000 : 3000) : -1
    ...
    const createToast = (config) => {
      ...
      const actionHtml = action?.href
        ? `<a href="${action.href}" class="button" data-toast-action>${action.label}</a>`
        : action?.onclick
          ? `<button type="button" data-toast-action onclick="${action.onclick}">${action.label}</button>`
          : ''
      const cancelHtml = cancel
        ? `<button type="button" data-variant="outline" data-toast-cancel onclick="${cancel?.onclick}">${cancel.label}</button>`
        : ''
...
customElements.define('bc-toaster', BcToaster)

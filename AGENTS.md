...
- Workspaces: root bun workspace with publishable packages in `packages/*`.
- Packages:
...
No automated testing configured. User validates manually via the docs site (`bun run docs:dev`).
...
- Always run `bun run build` before publishing packages

## Publishing packages

1. Bump version in root `package.json`
2. Run `bun run build` to generate dist files
3. Run `bun publish --workspaces` to publish both packages
